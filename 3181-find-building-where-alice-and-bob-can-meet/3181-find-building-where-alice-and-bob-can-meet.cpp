...
public:
    vector<int> leftmostBuildingQueries(vector<int>& heights, vector<vector<int>>& queries) {
        ...
        int index=1;//Segment Tree Index: Root at index-1 segment tree implementations

        buildSegmentTree(heights,0,n-1,index);
        vector<int> res;
        for(auto query: queries){
            ...
            while(low <= high){
                ...
                int rmq = rangeMaxQuery(heights,low,mid,0,n-1,index);
